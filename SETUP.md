...
> - 前後の空白は自動で除去される
> - **この打刻ツールを使うアカウントは入れないでください**（ご自身の別アカウントを含む）

**Agent は実メールアドレスを受け取る必要がない。** ユーザーが直接 GAS の画面に入力する。
`Config.gs` にアドレスを書き込んではならない（[ADR-0009](./ADR.md#adr-0009)）。

招待先は「予定を受け取って見るだけ」のアカウントに限る。招待された予定は招待先の
カレンダーにも載るため、招待先が打刻ツールを実行すると、他人の打刻を自分の打刻として
読み込んでしまう（→ [ADR-0025](./ADR.md#adr-0025)）。
